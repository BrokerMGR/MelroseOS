...
function MGR_LREC_139_parseSearchResponse(html, expectation) {
  ...
  if (!links.length) {
    // HARDENED RULE:
    // Absence of a hyperlink is NOT evidence of Pending.
    // Pending requires explicit LREC no-result wording above.
    return {
      success: false,
      recordFound: false,
      noResults: false,
      ...
      resultCount: 0,
      searchMode: expectation && expectation.mode || '',
      error: 'LREC_NO_LICENSE_LINK_WITHOUT_EXPLICIT_NO_RESULT',
      parserVersion: MGR_LREC_139.VERSION
    };
  }
