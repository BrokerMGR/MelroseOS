...
function MGR_LREC_143_parseAccountDetails_(text) {
  ...
  function matchOne(re) {
    const m = re.exec(t);
    return m ? String(m[1] || '').trim() : '';
  }

  const contactName = matchOne(
    /Contact Name\s+(.+?)\s+License Account Type\b/i
  );

  const accountType = matchOne(
    /License Account Type\s+(.+?)\s+Status\b/i
  );

  const licenseStatus = matchOne(
    /Status\s+([A-Za-z][A-Za-z ]*?)\s+License\s*#/i
  );

  const licenseNumber = matchOne(
    /License\s*#\s+([A-Z0-9.\-]+)\s+First Issued Date\b/i
  );

  const firstIssuedDate = matchOne(
    /First Issued Date\s+([0-9\/\-]+)\s+Expiration Date\b/i
  );

  const expirationDate = matchOne(
    /Expiration Date\s+([0-9\/\-]+)\s+(?:Education|Current Supervisor)\b/i
  );

  let companyName = matchOne(
    /Current Supervisor\s+Access\s+(.+?)\s+View Prior Supervisor History\b/i
  );

  if (!companyName) {
    companyName = matchOne(
      /Current Supervisor\s+(.+?)\s+View Prior Supervisor History\b/i
    );
  }
...
function RUN_LREC_143_3_PARSER_CERTIFICATION() {
  const sample =
    'Contact Name Clarissa Jessica Brown ' +
    'License Account Type Salesperson ' +
    'Status Active ' +
    'License # SALE.995720225-ACT ' +
    'First Issued Date 06/17/2026 ' +
    'Expiration Date 12/31/2026 ' +
    'Education View Education History ' +
    'Current Supervisor Access Real Estate Group Inc. ' +
    'View Prior Supervisor History';

  const parsed =
    MGR_LREC_143_parseAccountDetails_(sample);

  const result = {
    success:
      parsed.contactName ===
        'Clarissa Jessica Brown' &&
      parsed.accountType ===
        'Salesperson' &&
  ...
  console.log(
    'RUN_LREC_143_3_PARSER_CERTIFICATION\n' +
    JSON.stringify(result, null, 2)
  );
