...
function MGR_LREC_143_fetchDetails_(key, cookie) {
  ...
  const contact = MGR_LREC_143_fetch_(
    MGR_LREC_143.CONTACT_ENDPOINT + '?key=' + encoded + '&Source=Search',
    { method: 'get', cookie: cookie, referer: MGR_LREC_143.SEARCH_PAGE }
  );
  ...
  const qualifierText = qualifiers.success ? MGR_LREC_143_text_(qualifiers.body) : '';
  const contactText = contact.success ? MGR_LREC_143_text_(contact.body) : '';

  return {
    success: true,
    accountHttpCode: account.httpCode,
    qualifiersHttpCode: qualifiers.httpCode,
    contactHttpCode: contact.httpCode,
    licenseStatus: MGR_LREC_143_extractLabeledValue_(accountText, [
      'License Status', 'Status'
    ]),
    companyName: MGR_LREC_143_extractLabeledValue_(accountText, [
      'Company', 'Company Name', 'Brokerage'
    ]),
    accountDetails: accountText.substring(0, 1800),
    qualifiers: qualifierText.substring(0, 1200),
    contactDetails: contactText.substring(0, 1200)
  };
}
